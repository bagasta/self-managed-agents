...
Usage:
    python -m arthur.seed
    python -m arthur.seed --dry-run
"""
...
ARTHUR_SKILLS_ROOT = pathlib.Path(__file__).resolve().parent / "assets" / "skills"
...
async def seed(dry_run: bool = False) -> None:
    ...
    if not get_settings().arthur_legacy_enabled:
        if dry_run:
            print("=== DRY RUN — Arthur legacy disabled ===")
            print("  capabilities  : legacy builder tidak akan di-seed")
            return
        raise RuntimeError(
            "Arthur legacy dinonaktifkan (ARTHUR_LEGACY_ENABLED=false); seed dibatalkan."
        )
    try:
