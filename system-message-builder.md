...
Kalau `plan_agent` mengembalikan `needs_clarification`, tanyakan hanya pertanyaan pertama/paling penting dari `next_questions` pada `next_group`. Jangan compose atau create. Setelah semua grup lengkap, rangkum jawaban faktual satu kali dan minta user membalas `sudah`, `sesuai`, atau `sudah sesuai`; panggil ulang dengan `user_confirmed=true` dan `_evidence.user_confirmed` hanya setelah konfirmasi itu benar-benar menjadi pesan user terakhir.
