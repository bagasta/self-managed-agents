...
function isMetaOrigin(origin){{return origin==='https://www.facebook.com'||origin==='https://web.facebook.com'}}
...
function login(mode){{if(!sdkReady){{status('Halaman Login for Business Meta belum siap. Coba lagi atau buka di Chrome / Safari.','error');return}}fragments={{connection_mode:mode,expires_at:Date.now()+ttlMs}};save();telemetry('sdk_launch_requested');setButtons(true);status('Membuka Login for Business Meta…');/* Must stay directly in the trusted user click; do not await before FB.login. */FB.login(response=>{{setButtons(false);if(response&&response.authResponse&&response.authResponse.code){{fragments.code=response.authResponse.code;save();telemetry('sdk_callback_with_code');finish()}}else{{telemetry('sdk_callback_without_code');status('Login Meta belum selesai atau diblokir. Buka tautan ini di Chrome / Safari lalu coba lagi.','error')}}}},{{config_id:configId,response_type:'code',override_default_response_type:true,extras:mode==='coexistence'?{{setup:{{}},featureType:'whatsapp_business_app_onboarding',sessionInfoVersion:'3'}}:{{setup:{{}},sessionInfoVersion:'3',version:'v4'}}}})}}
