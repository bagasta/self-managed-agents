...
const key=`meta-es:${{state}}`,launchUrl=`/v1/meta/signup/l/${{encodeURIComponent(state)}}`,statusEl=document.querySelector('#status'),coexist=document.querySelector('#coexist'),newNumber=document.querySelector('#new-number'),selection=document.querySelector('#selection'),selectionOptions=document.querySelector('#selection-options');let sdkReady=false,submitting=false,fallbackTimer=null,fragments=read();
