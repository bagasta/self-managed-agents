...
async def launch(state: str = Query(..., min_length=32), db: AsyncSession = Depends(get_db)) -> str:
    ...
    mobile_oauth_url = "https://www.facebook.com/" + settings.meta_graph_api_version + "/dialog/oauth?" + urlencode({
...
#mobile-connect {{ display:none; width:100%; border-radius:13px; padding:15px 18px; background:linear-gradient(100deg,#1877f2,#2c65db); box-shadow:0 12px 25px rgba(24,119,242,.28); color:white; font:inherit; font-size:16px; font-weight:800; text-align:center; text-decoration:none; }}
...
@media (max-width:700px) {{ #connect {{ display:none; }} #mobile-connect {{ display:block; }} }}
...
connectButton.onclick=()=>{{if(fragments.completed)return;if(ready()){{attemptCompletion();return;}}connectButton.disabled=true;setStatus('Membuka Meta…');if(/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)){{window.location.assign(mobileOAuthUrl);return;}}if(!window.FB||typeof FB.login!=='function'){{connectButton.disabled=false;setStatus('Komponen Meta masih dimuat. Tunggu sebentar lalu coba lagi.','error');return;}}FB.login(response=>{{const returnedCode=response&&response.authResponse&&response.authResponse.code;if(returnedCode)receiveCode(returnedCode);else{{connectButton.disabled=false;setStatus('Menunggu konfirmasi Meta. Kembali ke halaman ini setelah setup selesai.');resumePending();}}}},{{config_id:{settings.meta_embedded_signup_config_id!r},response_type:'code',override_default_response_type:true,extras:{{setup:{{}}}}}});}};
resumePending();
