...
connectButton.onclick=()=>{{if(fragments.completed)return;if(ready()){{attemptCompletion();return;}}if(!window.FB||typeof FB.login!=='function'){{setStatus('Komponen Meta masih dimuat. Tunggu sebentar lalu coba lagi.','error');return;}}const mobile=/Android|iPhone|iPad|iPod/i.test(navigator.userAgent);connectButton.disabled=true;setStatus('Membuka Meta…');FB.login(response=>{{const returnedCode=response&&response.authResponse&&response.authResponse.code;if(returnedCode)receiveCode(returnedCode);else{{connectButton.disabled=false;setStatus('Menunggu konfirmasi Meta. Kembali ke halaman ini setelah setup selesai.');resumePending();}}}},{{config_id:{settings.meta_embedded_signup_config_id!r},response_type:'code',override_default_response_type:true,display:mobile?'touch':'popup',extras:{{setup:{{}}}}}});}};
resumePending();
