...
async def upsert_memory(
    ...
    After every write, the Redis cache entry for this key is invalidated so
    subsequent reads get the fresh value from PostgreSQL. The invalidation is
    best-effort: if Redis is down, the cache will expire via TTL naturally.
    """
    ...
    if scope is None:
        ...
        if existing:
            ...
            await db.flush()
            await _invalidate_memory_cache(agent_id, key, scope)
            return existing
...
async def _invalidate_memory_cache(
    ...
    """Fire-and-forget Redis cache invalidation after a memory write."""
    try:
        from app.core.infra.memory_cache import invalidate_memory_cache
        await invalidate_memory_cache(agent_id, key, scope=scope)
    except Exception:
        pass  # Cache miss on next read is safe — DB is always the source of truth
