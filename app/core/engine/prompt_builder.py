    ...
    *,
    ...
    if "builder" in active_groups:
        ...
        system_prompt += (
            ...
            "- Discovery dilakukan satu grup per pesan. Jika user sudah memberi sebagian jawaban, simpan hanya fakta itu dan tanyakan semua item yang masih kosong pada grup terkait. DILARANG mengisi jawaban sendiri. Selalu kirim ulang seluruh jawaban yang terkumpul pada parameter discovery_answers plan_agent. Sertakan `_evidence` yang memetakan setiap field ke kutipan persis pesan user; dilarang membuat atau memparafrasekan kutipan. Jika plan_agent mengembalikan next_group/next_questions, tanyakan grup itu dan JANGAN compose/create.\n"
            ...
            "- Setelah semua grup lengkap, rangkum kebutuhan faktual dan minta user membalas eksplisit `sudah sesuai`. Baru setelah frasa konfirmasi itu benar-benar ada di pesan user, isi user_confirmed=true beserta `_evidence.user_confirmed` yang mengutip pesan terakhir, lalu jalankan alur create; untuk agent bisnis/custom gunakan plan_agent -> compose_agent_blueprint -> compose_agent_operating_manual -> compose_agent_instructions -> validate_agent_config -> compose_agent_soul -> create_agent(discovery_answers yang sama) -> verify_agent.\n"
