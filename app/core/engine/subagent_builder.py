...
_SYSTEM_SUBAGENTS: list[dict] = [
    ...
    },
    {
        ...
        "system_prompt": (
            ...
            "1. Buat single-file /workspace/shared/index.html yang memuat seluruh CSS styling (<style> atau Tailwind CSS CDN) dan JavaScript (<script>).\n"
            "2. WAJIB timpa total (overwrite) menggunakan write_file agar tidak ada sisa kode website sebelumnya (DILARANG edit_file untuk project baru).\n"
            "3. Pastikan responsive, polished, berwarna indah, dan isi konten sesuai request user (bukan HTML polos tanpa CSS).\n"
            "4. Jika status tunnel lama masih berjalan, WAJIB stop_deployment() lalu deploy ulang untuk membuat tunnel baru.\n"
            "5. Deploy: deploy_app('python3 -m http.server 8080', 8080).\n"
            "6. Verifikasi dengan get_deployment_status().\n\n"
            ...
            "- Jangan `print` seluruh isi dokumen/log/dataset dari execute; tulis output besar ke file dan print ringkasan maksimal 50 baris.\n"
            "- Folder standar /workspace/shared, /workspace/tmp, /workspace/output, dan /workspace/data/incoming sudah ada; jangan buang langkah untuk mkdir folder standar.\n"
            ...
            "1. Tulis semua file via write_file secara UTUH, timpa bersih jika file sudah ada (DILARANG edit_file untuk project baru).\n"
            "2. Panggil get_deployment_status().\n"
            "3. JIKA project baru/deploy ulang: Panggil stop_deployment() lalu deploy_app(command, port) agar dapat URL fresh.\n"
            "4. JIKA sekadar revisi kecil: tulis dengan write_file, lalu jika status sudah 'running', JANGAN deploy ulang.\n"
            "5. Setelah deploy_app() selesai, verifikasi status dengan get_deployment_status() — pastikan URL aktif.\n"
            "6. Jika URL kosong, panggil get_deployment_logs() untuk debug, lalu perbaiki.\n"
