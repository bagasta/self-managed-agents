...
async def _deliver_shared_whatsapp_file_via_tool(
    *,
    ...
    if send_tool is None:
        ...
        existing_reply = parsed.get("final_reply", "")
        if existing_reply and len(existing_reply.strip()) > 0:
            return False, existing_reply
        return (
            False,
            f"File {filename} tersedia di workspace.",
        )
...
async def run_agent(
    *,
    ...
    async with mcp_client_context(mcp_tools_config) as (mcp_tools, mcp_errors):
        ...
        elif _is_builder_or_system_agent or _is_enabled(tools_config, "deploy", default=False) or _is_website_or_app_request(user_message):
            _timeout = min(settings.agent_timeout_seconds * 4, 600)
        else:
        ...
        try:
            ...
            if not _is_wa_session:
                timeout_reply = "Proses memakan waktu lebih lama dari biasanya (timeout). Silakan panggil get_deployment_status() atau minta agent melanjutkannya."
                db.add(Message(
                    session_id=session.id,
                    role="agent",
                    content=timeout_reply,
                    step_index=step_counter,
                    run_id=run_id,
                ))
                await db.commit()
