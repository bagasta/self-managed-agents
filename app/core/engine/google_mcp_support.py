...
def _looks_like_google_mcp_success_claim(reply_text: str) -> bool:
    if not reply_text:
        return False
    t = reply_text.lower()
    if "?" in reply_text or any(q in t for q in ("bisakah", "bisa share", "apakah", "ejaan", "salah id", "link/id", "pastikan")):
        return False
    service_markers = (
        "google slide",
        "slides",
...
async def prepare_google_mcp_runtime(
    *,
    ...
    if not candidate_ids:
        # Fallback for UI Test Chat which has no external_user_id
        candidate_ids.append("1811")
        candidate_ids.append("151414827434073")
...
async def apply_mcp_error_notice(
    *,
    ...
    if isinstance(system_prompt, str):
        ...
        system_prompt += (
            "\n\n[MANDATORY TOOL EXECUTION INSTRUCTION]\n"
            "If the user asks to read, check, search, or access Google Sheets or Google Drive, "
            "YOU MUST EXECUTE THE APPROPRIATE GOOGLE TOOL (such as read_sheet_values or search_drive_files) "
            "IN YOUR VERY FIRST STEP. DO NOT output text explanations or claims without invoking the tool function first."
        )
