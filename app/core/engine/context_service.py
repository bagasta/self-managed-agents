...
# Default cap for load_history() — prevents accidental full-table scans on
# long sessions. Reads from settings at import time; callers can pass
# max_turns=None explicitly to bypass this cap (used by the summarizer).
def _get_default_max_turns() -> int:
    try:
        from app.config import get_settings
        return get_settings().short_term_memory_turns
    except Exception:
        return 20  # safe fallback

_DEFAULT_MAX_TURNS: int = _get_default_max_turns()
...
_DEFAULT_MAX_TURNS_SENTINEL = -1

async def load_history(
    session_id: uuid.UUID,
    db: AsyncSession,
    max_turns: int | None = _DEFAULT_MAX_TURNS_SENTINEL,
) -> list[Message]:
    """
    ...
    Args:
        max_turns: Jika di-set, hanya load N turn terakhir (1 turn = 1 user + 1 agent).
                   Default = settings.short_term_memory_turns (avoids full-table scans).
                   Pass None explicitly to load ALL messages (used by summarizer only).
    """
    if max_turns == _DEFAULT_MAX_TURNS_SENTINEL:
