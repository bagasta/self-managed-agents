...
def to_host_path(internal_path: str | Path) -> str:
    ...
    - If `sandbox_host_base_dir` is unset, returns the resolved absolute path.
    - If `internal_path` is under `sandbox_base_dir`, returns the same relative location
      under `sandbox_host_base_dir`.
    - If `internal_path` is not under `sandbox_base_dir`, returns it resolved (best effort).
    """
    settings = get_settings()
    host_base = (settings.sandbox_host_base_dir or "").strip()
    if not host_base:
        return str(Path(internal_path).resolve())
