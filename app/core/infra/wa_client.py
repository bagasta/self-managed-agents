...
async def _get_cloud_access_token(phone_number_id: str) -> str | None:
    """Retrieve and decrypt Cloud API access token for a given phone_number_id."""
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(Agent.wa_access_token_encrypted).where(
...
async def send_wa_message(device_id: str, to: str, text: str) -> dict:
    ...
    Routing logic:
      - device_id starts with 'cloud_' → WhatsApp Cloud API (phone_number_id encoded after prefix)
      - device_id starts with 'wadev_' → wa-dev-service (shared demo number)
      - otherwise → legacy Go wa-service
    """
    if device_id.startswith("cloud_"):
        # Cloud API: device_id format is "cloud_{phone_number_id}"
        phone_number_id = device_id[len("cloud_"):]
        access_token = await _get_cloud_access_token(phone_number_id)
        if not access_token:
            logger.warning("wa_client.cloud_send.no_token", phone_number_id=phone_number_id)
            return {"status": "error", "error": "No access token for Cloud API phone number"}
        from app.core.infra.wa_cloud_client import send_text_message
        return await send_text_message(phone_number_id, to, text, access_token)
