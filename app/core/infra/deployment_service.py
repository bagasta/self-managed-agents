...
def _docker() -> docker.DockerClient:
    ...
    for sock in unique_candidates:
        if sock.startswith("unix://"):
            sock_path = sock.removeprefix("unix://")
        ...
        try:
            ...
            last_exc = exc
            continue
    raise RuntimeError(f"Cannot connect to Docker daemon. Last error: {last_exc}")
...
def deploy_app(
    ...
    if "http.server" in safe_command and not safe_command.startswith("cd ") and "--directory" not in safe_command:
        if (workspace_dir / "shared" / "index.html").exists():
            safe_command = f"cd /workspace/shared && {safe_command}"
        elif (workspace_dir / "src" / "index.html").exists():
            safe_command = f"cd /workspace/src && {safe_command}"
