...
def build_deployment_tools(session_id: str, workspace_dir: Path, sandbox_image: str) -> list:
    ...
    def deploy_app(command: str, port: int = 8080) -> str:
        ...
        1. Gunakan write_file untuk menulis semua kode (WAJIB timpa penuh /workspace/shared/index.html jika ada request website baru).
        2. Panggil get_deployment_status() untuk cek apakah sudah ada deployment
        3. JIKA user meminta aplikasi/website BARU, topik berbeda, atau meminta deploy ulang/fresh tunnel/buat ulang:
           - WAJIB panggil stop_deployment() terlebih dahulu untuk menghentikan tunnel lama.
           - Panggil deploy_app dengan command yang tepat.
        4. JIKA request HANYA revisi kecil/edit minor pada aplikasi yang SAMA:
           - Tulis perubahan ke /workspace/shared/index.html via write_file.
           - Jika status "running" → JANGAN deploy ulang, gunakan URL yang ada.
           - Jika "not_deployed" → panggil deploy_app.
        ...
        - Gunakan write_file untuk mengubah HTML/CSS/JS. TIDAK PERLU deploy_app lagi jika hanya edit/revisi.
        - deploy_app ulang WAJIB dilakukan jika project/website berganti total atau user minta fresh deploy.
