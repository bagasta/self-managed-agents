...
def build_builder_verify_tools(db_factory: async_sessionmaker) -> dict[str, Any]:
    ...
    async def verify_agent(agent_id: str) -> str:
        ...
        wa_is_ready = bool(getattr(agent, "wa_phone_number_id", None) or getattr(agent, "wa_device_id", None))
        if agent.channel_type == "whatsapp" and not wa_is_ready:
            readiness_warnings.append(
                "whatsapp_setup_required: agent WhatsApp belum terhubung ke Meta Embedded Signup (WABA)."
            )
