    ...
    *,
    ...
    async def compose_agent_instructions(
        ...
        if preset_id == "coding_deploy_agent":
            coder_note = (
                "\n\nCATATAN PENTING UNTUK CODING AGENT — WAJIB DIIKUTI PERSIS:\n"
                "Agent ini memiliki subagent sys_coder yang KHUSUS menulis kode, menjalankan di sandbox, dan deploy ke Cloudflare.\n\n"
                "ATURAN DELEGASI (jangan campurkan):\n\n"
                "1. REQUEST WEBSITE / WEB APP / LANDING PAGE / PORTFOLIO / TOOLS WEB:\n"
                "   → task() ke sys_coder\n"
                "   → sys_coder menulis HTML/CSS/JS ke workspace-nya, memanggil deploy_app(), dan MENGEMBALIKAN URL https://*.trycloudflare.com\n"
                "   → Main agent HANYA relay URL itu ke user\n"
                "   → JANGAN instruksikan main agent untuk kirim file HTML via WhatsApp — user butuh URL, bukan file\n"
                "   → JANGAN instruksikan main agent untuk panggil deploy_app() sendiri — sys_coder yang deploy\n"
                "   Contoh output sys_coder yang benar: 'Website sudah live di https://abc123.trycloudflare.com (aktif 24 jam)'\n"
                "   Contoh reply main agent ke user: 'Website-mu sudah live! Cek di: https://abc123.trycloudflare.com (aktif 24 jam)'\n\n"
                "2. REQUEST FILE DELIVERABLE (PDF, laporan, data, gambar, chart, script download):\n"
                "   → task() ke sys_coder atau sys_analyst\n"
                "   → subagent menyimpan file final ke /workspace/shared/<filename> dan return 'SIAP_DIKIRIM_PARENT: /workspace/shared/<filename>'\n"
                "   → Main agent panggil send_whatsapp_document() atau send_whatsapp_image() HANYA untuk jenis ini\n\n"
                "3. task description ke sys_coder harus RINGKAS (3-4 kalimat): nama output, teknologi, port jika web. JANGAN pseudocode panjang.\n"
                "4. JANGAN instruksikan main agent menulis kode sendiri — semua eksekusi kode via task() ke subagent.\n"
                "5. JANGAN campur pola deploy (→ URL) dengan file delivery (→ WhatsApp). Pilih sesuai jenis output:\n"
                "   - Perlu diakses online → deploy → relay URL\n"
                "   - Perlu disimpan/didownload → file → kirim via WhatsApp\n"
            )
