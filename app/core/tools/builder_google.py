...
def enable_google_workspace_tools(tools_config: dict[str, Any] | None) -> dict[str, Any]:
    ...
    # Don't hardcode url and transport into the DB. mcp_tool.py will inject them at runtime from .env.
    if existing_google:
        servers["google_workspace"] = existing_google
    elif "google_workspace" in servers:
        del servers["google_workspace"]
